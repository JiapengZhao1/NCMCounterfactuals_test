...
for root, dirs, files in os.walk(base_dir):
    for file in files:
        if file == "results.json":
            ...
            exp_info = path_parts[-1]
            # Use regex to extract graph name between 'graph=' and '-n_samples'
            graph_match = re.search(r'graph=(.*)-n_samples', exp_info)
            graph_name = graph_match.group(1) if graph_match else ""
            ...
            if isinstance(results, dict):
                ...
                results["dim"] = int(dim) if dim else None
                results_data.append(results)

# Convert the results data to a DataFrame
results_df = pd.DataFrame(results_data)

# 排序：先按 graph，再按 n_samples，再按 dim，再按 trial_index
results_df = results_df.sort_values(by=["graph", "n_samples", "dim", "trial_index"])

# Reorder columns for better readability
columns_order = [
    "graph", "n_samples", "dim", "trial_index",
    "avg_error", "elapsed_time_2"
]
